...
internal sealed class InMemoryLeaderboardStore : ILeaderboardStore
{
    ...
    public decimal UpdateScore(long customerId, decimal delta)
        => _scores.AddOrUpdate(customerId, delta, (_, old) => old + delta);

    public bool TryGetScore(long customerId, out decimal score)
        => _scores.TryGetValue(customerId, out score);
    ...
    public void RefreshActive(long customerId, decimal newScore)
    {
        ...
        try
        {
            ...
            {
                _tree.Remove(node!);
                _active.Remove(customerId);
            }
            else if (!active && shouldBe)
            {
                var n = _tree.Insert(customerId, newScore);
                _active[customerId] = n;
            }
            else if (active && shouldBe)
            {
                _tree.Remove(node!);
    ...
    public LeaderboardEntry? GetByRank(int rank)
    {
        ...
        try
        {
            var node = _tree.SelectByRank(rank);
            if (node == null) return null;
            return new LeaderboardEntry(node.CustomerId, node.Score, rank);
        }
        finally { _lock.ExitReadLock(); }
    }
    ...
    public int GetRank(long customerId)
    {
        ...
        try
        {
            if (!_active.TryGetValue(customerId, out var node)) return 0;
            return _tree.GetRank(node);
        }
        finally { _lock.ExitReadLock(); }
    }
}